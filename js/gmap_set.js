...
function initialize() {
    ...
    //controlDiv2.index = 1;
    //map.controls[google.maps.ControlPosition.BOTTOM_RIGHT].push(controlDiv2);
...
function PleaceMarker(location, updateAddress) {
    ...
    ShowInfoWindow(setmarker, "set the place here!<br> 拖动图标选择您的位置，或者直接右键选择位置");
...
function updateMarkerPosition(latLng) {
    if (document.getElementById('hse_lat'))
        document.getElementById('hse_lat').innerHTML = latLng.lat();
    if (document.getElementById('hse_lng'))
        document.getElementById('hse_lng').innerHTML = latLng.lng();
