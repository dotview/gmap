...
$.widget( "ui.GeoAutocomplete", {
	...
	_init: function() {
		...
		self.element.focus(function(){
				if(this.value==self.options.hintInput){
				this.value="";}
			}).blur(function(){
				if(this.value==""){
				this.value=self.options.hintInput;
				}
		});
		self.element.val(self.options.hintInput);
	},
	...
	options: {
		...
		hintInput:'attractions, shops, restaurants, bars and clubs',//
