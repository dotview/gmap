<link rel="stylesheet" href="/dev/wp-content/themes/yoo_shelf_wp/css/map.css" />
<div>
map title: <? the_field("map_title");echo '<br>'; ?>
map zoom: <? the_field("map_zoom"); echo '<br>'; ?>
map display type: <? the_field("map_display_type"); echo '<br>'; ?>
places category: <? the_field("places_category");  echo '<br>'; ?>
...
<div id="map_canvas"></div>
		...
		<script src="/dev/wp-content/themes/yoo_shelf_wp/js/map2.js"></script>
		<script>
		...
		<?php
			...
			foreach($catearray as $category) { 
				...
				$result .= '{"category":"'.$category_ID.'","categoryname":"'.$category_Name.'","permalink":"'.get_category_link( $category_ID ).'","list":[';
				// THIS IS THE WORDPRESS QUERY LOOP //
				
				$the_query = new WP_Query( 'cat='.$category_ID.'&orderby=title&order=ASC&posts_per_page=50' );
