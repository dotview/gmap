...
Guiderer.Views.Location = (function(_super) {
  ...
  Location.prototype.initialize = function() {
    var guide_Location_val;
    this.searchField = $('#searchTextField');
    this.searchField.keypress(function(e) {
      return e.keyCode !== 13;
    });
    guide_Location_val = $('#guide_location').val();
    /*if (guide_Location_val !== "") {
      this.MapSearch(guide_Location_val, this.searchCallback);
    }*/
	try{
	$('#poi_name').geo_autocomplete(new google.maps.Geocoder, {
  ...
  Location.prototype.getCountryShortName = function(place) {
	var addrs = place.address_components;
	for(var i=0;i<addrs.length;i++){
		var ts = addrs[i].types;
		for(var j=0;j<ts.length;j++){
			var type = ts[j];
			if(type=="country"){
				return addrs[i].short_name;
			}
		}
	}
	return "";
  ...
  Location.prototype.placeChanged = function() {
    ...
    if (place.geometry.viewport) {
      ...
      /*options = {
        ...
        types: ['establishment'],
		componentRestrictions: {country: this.getCountryShortName(place)}
      };
      this.autocomplete_guide = new google.maps.places.Autocomplete($('#poi_name')[0], options);*/
    } else {
...
function() {
    ...
    a = function() {
		...
		var e= new Guiderer_Views_PoiMap;
