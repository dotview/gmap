...
var currQueueNum = 0;
	...
	if (chunkNode < wayArr.length) {
		...
		travelMode: travelMode }, 
				  function(directionsResult, directionsStatus) {
					  ...
					  if (typeof onNoRouteCallback == 'function') {
						onNoRouteCallback(errorMsg);
					  }
				      } else {
	...
	function geocodePosition(pos,callback) {
		gebGeocoder.geocode({
			...
			if (responses && responses.length > 0) {
				respAddress = responses[0].formatted_address;
				if (typeof callback == 'function'){
	...
	function addDragMarker(latlng,callback){
		if(latlng==null){
			latlng= gebMap.getCenter();
		}
		var setmarkerImage = "images/marker0.png";
		...
		if (typeof callback == 'function'){
			google.maps.event.addListener(setmarker, 'dragend', function() {
				geocodePosition(setmarker.getPosition(),callback);
			});
			google.maps.event.addListener(gebMap, 'rightclick', function(event) {
				setmarker.setPosition(event.latLng);
				geocodePosition(event.latLng,callback);
			});
		}
	}
    ...
    BpTspSolver.prototype.solveRoundTrip = function(callback,failcallback) {
	if (doNotContinue) {
	...
	if (!this.isReady()) {
	    setTimeout(function(){ tsp.solveRoundTrip(callback,failcallback) }, 20);
	    return;
	}
	if (typeof callback == 'function')
	    onSolveCallback = callback;
	if (typeof failcallback == 'function')
	    onNoRouteCallback = failcallback;
		
	directions(0);
    };

    BpTspSolver.prototype.solveAtoZ = function(callback,failcallback) {
	if (doNotContinue) {
	    alert('Cannot continue after fatal errors.');
	    return;
	}
	...
	if (!this.isReady()) {
	    setTimeout(function(){ tsp.solveAtoZ(callback,failcallback) }, 20);
	    return;
	}
	...
	//
	BpTspSolver.prototype.addDragMarker = function (callback) {
		if (typeof callback == 'function')
