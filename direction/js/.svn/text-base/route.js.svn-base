...
function getRoute() {
    ...
    $.ajax({
        ...
        success: function(d) {
            if (d.result == 0) {
                $("#directions_panel").html("No route data in the day");
            } else {
                dayroutes = d.data;
                calcRoute();
            }
        ...
        error: function(d) {
            $("#directions_panel").html("route data error");
        }
    })
}
function reset() {
...
function calcRoute() {
    ...
    if (dayroutes.length == 1 ) {
        codeAddress(dayroutes[0],icons[0],null);
        showAppointment(dayroutes);
        return;
    }
	var start = "";
	var end = "";
	...
	for (var i = 0; i < dayroutes.length; i++) {
		if(dayroutes[i].found && start==""){
			start = decodeURI(dayroutes[i].address);
		}
		if(dayroutes[i].found){
			end = decodeURI(dayroutes[i].address);
    ...
    if ( newdayroutes.length == 1) {
        codeAddress(newdayroutes[0],icons[0],null);
        showAppointment(newdayroutes);
        return;
    }
    ...
    if (newdayroutes.length > 2) {
        ...
        for (var i = 1; i < newdayroutes.length - 1; i++) {
            waypoints.push({
                location: decodeURI(newdayroutes[i].address),
                stopover: true
            })
        }
...
function showRoute(response) {
	...
	//showAppointment_None(legs.length-1);
	var k =legs.length;
	for (var i = 0; i < nonedayroutes.length; i++) {
		k+=1;
		html.push(getPlaceHtml(nonedayroutes[i],icons[k],false));
   }
...
function showAppointment(routes) {
    ...
    for (var i = 0; i < routes.length; i++) {
		html.push(getPlaceHtml(routes[i],icons[i],false));
	}
    $("#directions_panel").html(html.join(""));
}

function getPlaceHtml(route,icon,ismarker) {
